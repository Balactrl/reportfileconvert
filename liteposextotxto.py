    ...
    if submitted:
        ...
        with st.spinner("Extracting sales data and building Excel file..."):
            try:
                ...
                st.success(f"Excel file generated: {output_path.name}")
                st.download_button(
...
def build_excel_to_txt():
    ...
    with st.form("excel_to_txt_form"):
        ...
        st.caption("All generated files will be saved to an output folder.")
    ...
    if submitted:
        ...
        output_folder = Path(tempfile.gettempdir()) / "Excel_to_TXT_Output"
        ...
        with st.spinner(f"Converting {len(uploaded_excel_files)} file(s) to TXT..."):
            ...
            for idx, uploaded_excel in enumerate(uploaded_excel_files):
                try:
                    ...
                    saved_path, report_text = excel_to_text(temp_excel_path, str(output_file_path))
        ...
        st.success(f"Successfully converted {success_count} file(s) to: {output_folder.name}")
        ...
        st.info(f"Output folder location: {output_folder.absolute()}")
        ...
        with st.expander("Preview first file"):
            if output_files:
                ...
                with open(first_file, "r", encoding="utf-8") as f:
                    preview_text = f.read()
                st.text_area("First TXT Preview", preview_text, height=300)
